  ...
  return (
    <div className={classes.root} ref={rootRef} onBlur={(event) => {
      ...
      >
        {label}
        {single
          ? <span className={classes.count} data-count-kind="value">{selected[0]}</span>
          : narrowed.length > 0 && <span className={classes.count} data-count-kind="number">{narrowed.length}</span>}
